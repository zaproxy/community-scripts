    ...
    if((cookieIndex = ccList.indexOf(cc.getName())) >= 0)
    {
      ...
      for(loopCounter=0; loopCounter < cookieList.length; loopCounter++) {
        if((equalsIndex=cookieList[loopCounter].indexOf('=')) > 0) {
          ...
          helper.addParamQuery(ccList[cookieIndex]+':'+decodeURIComponent(cookieList[loopCounter].substring(0,equalsIndex)),decodeURIComponent(cookieList[loopCounter].substring(equalsIndex+1)));
        }
      }
    }
  }
...
function setParameter(helper, msg, param, value, escaped)
{
  ...
  var prefix;
  var val;
  if((pos < size) && ((colonIndex = (paramName = helper.getParamName(pos)).indexOf(':')) > 0) &&
     (ccList.indexOf((cookieName = paramName.substring(0,colonIndex))) >= 0))
  {
    ...
    for(loopCounter=0; loopCounter<size; loopCounter++) {
      if(loopCounter == pos) {
        val = encodeURIComponent(helper.getParamName(loopCounter).slice(colonIndex+1)) + "=" + encodeURIComponent(value) + "&" + val;
      }
      else if((paramName = helper.getParamName(loopCounter)).startsWith(prefix))
      {
        val = encodeURIComponent(paramName.slice(colonIndex+1)) + "=" + encodeURIComponent(helper.getParamValue(loopCounter)) + "&" + val;
      }
    }
    /* remove trailing '&' */
    val = val.slice(0,-1);
    ...
    print('CompoundCookie SetParameter Error: Invalid input ' + size + ', ' + pos + ' -> ' + paramName);
  }
}
